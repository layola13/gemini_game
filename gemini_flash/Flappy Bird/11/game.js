...
const INVINCIBLE_TIME = 50; // 无敌时间
const PIPE_GENERATE_INTERVAL = 1500; // 管道生成间隔 (毫秒)
const DIFFICULTY_SETTINGS = {
  easy: {
    pipeGapDecreaseRate: 0.1,
    pipeSpeed: 1.5,
  },
  medium: {
    pipeGapDecreaseRate: 0.25,
    pipeSpeed: 2,
  },
  hard: {
    pipeGapDecreaseRate: 0.5,
    pipeSpeed: 2.5,
  }
};
...
let lastPipeGenerationTime = 0; // 上一次生成管道的時間
...
birdImage.src = 'bird-flap.png';
birdImage.onerror = () => {
  birdImage.src = 'default-bird.png'; // 加载失败时使用默认图片
};

const backgroundImage = new Image();
backgroundImage.src = 'background.png'; // 替换为你的背景图片路径
...
class Pipe {
  constructor(x, height, gap) {
    ...
    this.gap = gap;
    this.speed = DIFFICULTY_SETTINGS[difficulty].pipeSpeed;
  }
...
function generatePipe() {
  // 动态调整管道间距
  let gap = 150;
  gap -= Math.floor((Date.now() / 1000 - gameStartTime) * DIFFICULTY_SETTINGS[difficulty].pipeGapDecreaseRate); 
...
function handleInput(event) {
  if (event.code === 'Space' || event.code === 'ArrowUp') {
    ...
    if (gameState === 'running') {
      gameState = 'paused';
      pauseButton.textContent = '继续'; // 更改按钮文字
    } else if (gameState === 'paused') {
      gameState = 'running';
      pauseButton.textContent = '暂停'; // 更改按钮文字
      update();
    }
  }
}
...
function update() {
  ...
  if (gameState === 'running') {
    ...
    // 生成管道
    if (Date.now() - lastPipeGenerationTime >= PIPE_GENERATE_INTERVAL) {
      generatePipe();
      lastPipeGenerationTime = Date.now();
    }
  }
