...
const BIRD_JUMP_FORCE = -8;
const PIPE_WIDTH = 50;
const PIPE_SPEED = 2;
const PIPE_MIN_GAP = 100; // 最小管道间距
...
function generatePipe() {
  ...
  if (difficulty === 'medium') {
    gap -= Math.floor((Date.now() / 1000 - gameStartTime) * 0.25); // 每秒缩小0.25
  } else if (difficulty === 'hard') {
    gap -= Math.floor((Date.now() / 1000 - gameStartTime) * 0.5); // 每秒缩小0.5
  }
...
function checkCollision(bird, pipe) {
  // 使用圆形碰撞检测
  return (
    Math.sqrt(Math.pow(bird.x - (pipe.x + pipe.width / 2), 2) + Math.pow(bird.y - (pipe.height + pipe.gap / 2), 2)) < bird.radius
  );
}
