...
export class Pinball {
  constructor(canvas, ctx) {
    ...
    // 障碍物
    this.obstacles = [
      { x: 150, y: 150, width: 60, height: 20, color: 'blue' },
      { x: 350, y: 250, width: 80, height: 30, color: 'orange' },
      { x: 550, y: 300, width: 50, height: 20, color: 'green' }, 
      { x: 200, y: 350, width: 70, height: 25, color: 'purple' },
      { x: 500, y: 100, width: 90, height: 35, color: 'yellow' }
    ];
    ...
    // 添加音效
    this.paddleHitSound = this.loadSound('paddle_hit.wav'); // 球拍击球音效
    this.obstacleHitSound = this.loadSound('obstacle_hit.wav'); // 障碍物碰撞音效
    this.gameOverSound = this.loadSound('game_over.wav'); // 游戏结束音效
  }

  loadSound(src) {
    const sound = new Audio(src);
    sound.onerror = () => {
      console.error(`音效加载失败: ${src}`);
    };
    return sound;
  }
