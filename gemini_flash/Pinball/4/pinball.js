...
export class Pinball {
  ...
  gameOver() {
    ...
    this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height); // 添加半透明背景
    this.ctx.font = '30px Arial';
    this.ctx.fillStyle = "white"; // 设置文字颜色为白色
    this.ctx.fillText('游戏结束', this.canvas.width / 2 - 100, this.canvas.height / 2 - 30);
    this.ctx.fillText('得分: ' + this.score, this.canvas.width / 2 - 50, this.canvas.height / 2);
  }
  ...
  draw() {
    ...
    this.ctx.fillStyle = 'blue'; // 修改球的颜色
    this.ctx.fill();
    this.ctx.closePath();
